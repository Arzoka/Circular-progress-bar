window.onload = function () {
  ...
  const doAnimation = (
    ...
    if (temp_progress <= progress) {
      setTimeout(() => {
        progress_bar.style.background = `radial-gradient(closest-side, ${middleColor} 79%, transparent 80% 100%), conic-gradient(${lineColor} ${temp_progress}%, ${bgColor} 0)`;
        doAnimation(
          progress_bar,
          progress,
          lineColor,
  ...
  Array.from(circular_progress_bars).forEach((progress_bar) => {
    ...
    const middleColor = progress_bar.getAttribute("middle-color");
